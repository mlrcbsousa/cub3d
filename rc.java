...
public class Rayc extends Applet implements Runnable {
  ...
  public void createTables() {
    ...
    for (i = 0; i <= ANGLE360; i++) {
      ...
      if (i >= ANGLE90 && i < ANGLE270) {
        ...
        if (fXStepTable[i] > 0) fXStepTable[i] = -fXStepTable[i];
      }
      // facing right
      else {
        fXStepTable[i] = (float) (TILE_SIZE / fTanTable[i]);
      ...
      if (i >= ANGLE0 && i < ANGLE180) {
        ...
        if (fYStepTable[i] < 0) fYStepTable[i] = -fYStepTable[i];
      }
      // FACING UP
      else {
        fYStepTable[i] = (float) (TILE_SIZE * fTanTable[i]);
  ...
  public void run() {
    ...
    while (true) {
      // rotate left
      if (fKeyLeft) {
        if ((fPlayerArc -= ANGLE10) < ANGLE0) fPlayerArc += ANGLE360;
      }
      // rotate right
      else if (fKeyRight) {
        if ((fPlayerArc += ANGLE10) >= ANGLE360) fPlayerArc -= ANGLE360;
      }
  ...
  public void drawOverheadMap() {
    ...
    for (int u = 0; u < MAP_WIDTH; u++) {
      for (int v = 0; v < MAP_HEIGHT; v++) {
        ...
        fOffscreenGraphics.fillRect(
          PROJECTIONPLANEWIDTH + (u * fMinimapWidth),
          (v * fMinimapWidth),
          fMinimapWidth,
          fMinimapWidth
        );
    ...
    fPlayerMapX =
      ...
      (int) (((float) fPlayerX / (float) TILE_SIZE) * fMinimapWidth);
    fPlayerMapY =
      (int) (((float) fPlayerY / (float) TILE_SIZE) * fMinimapWidth);
  }
  ...
  public void drawRayOnOverheadMap(float x, float y) {
    ...
    fOffscreenGraphics.drawLine(
      ...
      fPlayerMapY,
      (int) (
        PROJECTIONPLANEWIDTH + ((float) (x * fMinimapWidth) / (float) TILE_SIZE)
      ),
  ...
  public void render() {
    ...
    for (castColumn = 0; castColumn < PROJECTIONPLANEWIDTH; castColumn += 5) {
        ...
        while (true) {
          ...
          if (
            ...
            distToVerticalGridBeingHit =
              (yIntersection - fPlayerY) * fISinTable[castArc];
            break;
          } else {
      ...
      int projectedWallHeight = (int) (
        WALL_HEIGHT * (float) fPlayerDistanceToTheProjectionPlane / dist
      );
      bottomOfWall =
        fProjectionPlaneYCenter + (int) (projectedWallHeight * 0.5F);
      topOfWall = PROJECTIONPLANEHEIGHT - bottomOfWall;
      if (bottomOfWall >= PROJECTIONPLANEHEIGHT) bottomOfWall =
